package sidev.app.course.dicoding.bab3_modul3.githubuser3.provider
...
class UserFavProvider: ContentProvider() {
    ...
    private val matcher: UriMatcher by lazy {
        UriMatcher(UriMatcher.NO_MATCH).apply {
            for(uri in Const.UserFavUri.values())
                addURI(Const.PROVIDER_AUTH, uri.uri, uri.ordinal +1)
        }
    }

    private val dao: UserFavDao by lazy {
        UserFavDb.getInstance(context!!).dao()
    }
